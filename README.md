...
Мы собираемся написать простую домашнюю страницу с блогом.  Это будет наша первая связь «многие-ко-многим» в контексте программы Express.  Позже мы 
...
Подумайте о ваших маршрутах (routers).  Проанализируйте создание трех файлов:
...
### Release 1: Модели &amp; Валидации (Если вы используете Sequelize)

Создайте все необходимые модели и миграции для поддержки вышеизложенного. У вас должно быть три модели, что они собой представляют?

Добавьте соответствующие валидации в свои модели.

Например, если в вашей модели `Entry` есть поля `body` и `title`, вы, вероятно, не хотите разрешать сохранение `Entry`, у которой нет полей `body` и `title`, в базе данных. Это означает добавление ограничений `NOT NULL` для миграций и последующих валидаций в вашу модель Sequelize:
Какие поля вы хотите, чтобы были у вашей записи в блоге? Автор?
...
Все ваши routings должны теперь реализовать свои основные функции. Вы должны иметь возможность создавать списковую структуру, показывать, создавать, обновлять и удалять записи. Вы также должны иметь возможность видеть все записи для данного тега по URL-адресу, например
...
Используя `try/catch/finally, throw`, убедитесь, что вы корректно обрабатываете свои ошибки. Обработка не обязательно должна быть идеальной, но хорошая обработка случая ошибки означает следующее: 
